...
<%		
        ...
        String temp = new String(request.getParameter("p_id").getBytes("ISO8859_1"),"utf-8");
        String amounts = new String(request.getParameter("amounts").getBytes("ISO8859_1"),"utf-8");
		int id = Integer.parseInt(temp);//商品ID，商品的唯一标识符
		int new_amount;
	      //连接数据库的基本操作
		...
		if(conn != null){
			...
			ResultSet rs = stmt.executeQuery(sql);			
				...
				if(balance>=price*amount){//判断用户余额是否大于商品总价格
		        ...
		        String sql_u1="Update product_message set ID=?,NAME=?,PRICE=?,TYPE=?,SUB=?,SUBMITTIME=?,REGION=?,AMOUNT=? where ID='"+id+"' ";
		        String sql_u2 = "select * from product_message where ID='"+id+"' ";//更新商品信息，数量--
		        ...
		        //删除购物车信息
		        String sql_c1="Update personal_cart set ID=?,NAME=?,PRICE=?,AMOUNT=?,SELLER=?,CLIENT=?,PRODUCTID=? where PRODUCTID='"+id+"' AND CLIENT='"+user+"'";
		        String sql_c2 = "select * from personal_cart where PRODUCTID='"+id+"' AND CLIENT='"+user+"'";//更新商品信息，数量-1
